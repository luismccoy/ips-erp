...
class ReasoningService {
  ...
  async processText(text, model = "us.anthropic.claude-3-5-sonnet-20241022-v2:0") {
  ...
  async processWithBedrock(text, model) {
    ...
    if (!accessKeyId || !secretAccessKey) {
      throw new Error("AWS credentials not configured. Go to Settings → AI Models to add your AWS credentials.");
    }

    // Configure BedrockService with credentials
    BedrockService.setCredentials(accessKeyId, secretAccessKey);
    BedrockService.setRegion(region);

    // Build the prompt
    const agentName = getAgentName();
    const prompt = getReasoningPrompt(text, agentName);
    const maxTokens = Math.max(100, text.length * 2);

    // Call Bedrock via the Bedrock Runtime API
    ...
    // Use the electron API if available, otherwise make direct call
    if (window.electronAPI?.invokeBedrockModel) {
      ...
      return result?.content?.[0]?.text?.trim() || result;
    }

    // Fallback: Direct API call with AWS Signature V4
    const response = await this.signedBedrockRequest(endpoint, body, {
      accessKeyId,
      secretAccessKey,
      region,
      model
  ...
  async signedBedrockRequest(endpoint, body, credentials) {
    const { accessKeyId, secretAccessKey, region, model } = credentials;
  ...
  async getAPIKey(provider) {
    ...
    switch (provider) {
      ...
      case "anthropic":
        ...
        break;
      case "bedrock":
        // For bedrock, we return credentials object instead of single key
        const accessKeyId = localStorage.getItem("awsAccessKeyId");
        const secretAccessKey = localStorage.getItem("awsSecretAccessKey");
        if (accessKeyId && secretAccessKey) {
